...
class ParallaxHighSpeedContinuousServoMotor(ContinuousServoMotor):
    def __init__(self, voltage: Real, dead_zone: Tuple[Real, Real] = None):
        ...
        hard_max_rpm = scale(voltage, Point(6, 150), Point(7.4, 180))
    ...
    def map_speed_to_pwm_us(self, speed: Real) -> Real:
        ...
        """
        full_cw_point = Point(1, self.full_cw_pwm_us)
        near_full_cw_point = Point(0.88, 1420)
        high_dead_point = Point(0, self.dead_zone[1])
        low_dead_point = Point(0, self.dead_zone[0])
        near_full_ccw_point = Point(-0.88, 1580)
        full_ccw_point = Point(-1, self.full_ccw_pwm_us)

        if speed >= 1:
            return self.full_cw_pwm_us
        elif speed >= near_full_cw_point.x:
        ...
        elif speed >= near_full_ccw_point.x:
